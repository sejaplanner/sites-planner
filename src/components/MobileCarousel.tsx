...
const MobileCarousel = () => {
  ...
  return <div className="md:hidden relative">
      ...
      <div className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10">
        <Button variant="outline" size="icon" onClick={prevSlide} className="w-8 h-8 bg-white/80 backdrop-blur-sm hover:bg-white/90">
          <ChevronLeft className="w-4 h-4" />
        </Button>
      </div>
      
      <div className="absolute right-2 top-1/2 transform -translate-y-1/2 z-10">
        <Button variant="outline" size="icon" onClick={nextSlide} className="w-8 h-8 bg-white/80 backdrop-blur-sm hover:bg-white/90">
        ...
        transform: `translateX(-${currentSlide * 100}%)`
          ...
          return <div key={index} className="w-full flex-shrink-0 px-4">
                <Card className={`p-4 bg-white/70 backdrop-blur-sm ${card.borderColor} hover:shadow-lg transition-all duration-300 h-44`}>
                  ...
                  <p className="text-gray-600 leading-relaxed py-0 my-0 mx-0 px-0 text-xs font-light">
                    {card.description}
                  </p>
                </Card>
              </div>;
